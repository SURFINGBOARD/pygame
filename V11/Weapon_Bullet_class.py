...
class BulletSP(MySprite):
    ...
    def __init__(self,position,Bullet_type=0,bullet_file="Pictures/weapons/bullet_0.png"):
        MySprite.__init__(self,screen)
        self.Bullet_type=Bullet_type
    ...
    def update(self,ticks):
        ...
        if self.Bullet_type==0:
            ...
            if self.float_pos.x<0 or self.float_pos.x>900\
                or self.float_pos.y<0 or self.float_pos.y>600:
                self.alive=False
            self.rotation=wrap_angle(self.rotation)
            angle=self.rotation+90
            self.scratch=pygame.transform.rotate(self.master_image, -angle)
    ...
    def draw(self,surface):
        if self.Bullet_type==0:
            ...
            width,height=self.scratch.get_size()
            center=Point(width/2,height/2)
            self.scratch=pygame.transform.scale(self.scratch, (int(width * 4),int(height * 4)))
            surface.blit(self.scratch,(self.X-center.x,self.Y-center.y))
        elif self.Bullet_type==2:
            width,height=self.scratch.get_size()
            center=Point(width/2,height/2)
...
'''           
    ...
    def fire_cannon(charactor):
        position=Point(charactor.X,charactor.Y)#替换成发射子弹处的位置
        bullet=Bullet(position)###############set condition for different Bullet_i or set a parameter to choose differen kinds of Bullet_i
    ...
    def player_fire_cannon():
        bullet=Bullet.fire_cannon(weapon)# derive parameter for changing Bullets_i  
        bullet.owner="weapon"
        bullet.color=(30,250,30)
        
    def enemy_cannon(charactor):
        position=Point(charactor.X,charactor.Y)
        bullet=Bullet_3_double(position)###############set condition for different Bullet_i or set a parameter to choose differen kinds of Bullet_i
        angle=charactor.weapon.rotation
        bullet.velocity=angular_velocity(angle)
        bullets.append(bullet)
        #play_sound(shoot_sound)
        return bullet
...
def game_init():#保留bullets 和Bullet_type为子弹，crosshair，crosshair_group为准心，其他应该在整合后主程序里都有包含,本方法应整合进主程序
